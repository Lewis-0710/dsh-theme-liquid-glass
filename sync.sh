#!/usr/bin/env bash
#==============================================================================
# sync.sh — dsh-theme-liquid-glass 上游同步脚本
#==============================================================================
#
# 功能：
...
#     2. 运行自动化测试与构建验证（npm run build && npm test）
...
if ! git remote get-url upstream >/dev/null 2>&1; then
    ...
    echo "💡 请先运行: git remote add upstream https://github.com/FAVKTOXIC/dsh-theme-liquid-glass.git"
    exit 1
fi
...
# Step 0: 创建安全临时备份目录
TMP_DIR="$(mktemp -d /tmp/dsh_theme_liquid_glass_sync_XXXXXX)"
cleanup() {
    rm -rf "$TMP_DIR"
}
trap cleanup EXIT INT TERM

# 优先从当前 custom 分支提取最新定制差异；若不存在则使用已有 sync.patch
if git rev-parse --verify custom >/dev/null 2>&1 && ! git diff --quiet upstream/main...custom ':!sync.patch' ':!sync.sh' 2>/dev/null; then
    git diff upstream/main...custom ':!sync.patch' ':!sync.sh' > "$TMP_DIR/sync.patch" 2>/dev/null || true
